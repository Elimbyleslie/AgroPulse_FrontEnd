...
const EmailVerification: React.FC = () => {
  ...
  // ─── Envoi initial OTP — UNE SEULE FOIS grâce au ref ─────────────────────
  const hasSentRef = useRef(false);

  useEffect(() => {
    if (!userEmail || hasSentRef.current) return;
    hasSentRef.current = true;

    dispatch(resendOtp({ email: userEmail }))
      .unwrap()
      .then(() => setCountdown(60))
      .catch((err: any) => console.error("Initial OTP send failed:", err));
  }, [dispatch, userEmail]);
  ...
  const handleVerify = async (
    ...
    if (!userEmail) { toast.error("Email manquant"); setSubmitting(false); return; }

    try {
      await dispatch(verifyEmailOTP({ otp: values.otp, email: userEmail })).unwrap();
      toast.success("Email vérifié avec succès ! Vous pouvez maintenant vous connecter.");
      setTimeout(() => navigate(PATH_AUTH.LOGIN, { replace: true }), 1500);
    } catch (error: unknown) {
      const getErr = (err: unknown): { message: string; code?: number } => {
        if (err && typeof err === "object") {
          const message =
            "message" in err && typeof err.message === "string"
              ? err.message
              : "meta" in err && err.meta && typeof err.meta === "object" && "message" in err.meta
              ? String((err.meta as any).message)
      ...
      const { message: msg, code } = getErr(error);

      if (code === 400 || msg.toLowerCase().includes("invalide")) {
        toast.error("Code OTP invalide");
      } else if (code === 410 || code === 404 || msg.toLowerCase().includes("expiré")) {
  ...
  const handleResend = async () => {
    if (!userEmail) { toast.error("Email manquant"); return; }
    ...
    try {
      ...
      if (error?.meta?.status === 500 || error?.status === 500) {
        toast.error("Le serveur rencontre une difficulté, vérifiez votre boîte mail avant de réessayer.");
      } else {
        toast.error(error?.meta?.message || "Erreur de connexion");
      }
    } finally {
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <div className="flex justify-center mb-4">
          <div className="bg-green-100 p-4 rounded-full">
            <svg className="w-12 h-12 text-vert" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <h2 className="text-2xl font-bold mb-2 text-center text-gray-800">Vérification de l'email</h2>
        <p className="text-gray-600 mb-4 text-center text-sm">
          Nous avons envoyé un code de vérification à 6 chiffres à l'adresse :
        </p>
        {userEmail && (
          <p className="text-center mb-6">
            <strong className="text-vert">{userEmail}</strong>
          </p>
        )}
          ...
          {({ isSubmitting, values }) => (
            <Form className="flex flex-col gap-4">
              ...
              <Button
                ...
                className="w-full py-3 bg-vert hover:bg-[#0a6d3a] flex justify-center text-white font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
        ...
        <div className="mt-6 text-center">
          ...
          <button
            ...
            className="text-vert hover:text-[#0a6d3a] font-medium text-sm hover:underline disabled:opacity-50 disabled:cursor-not-allowed disabled:no-underline transition-colors"
          >
            {resendLoading ? (
