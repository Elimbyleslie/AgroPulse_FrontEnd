...
const initialFormValues: FormValues = useMemo(() => {
  ...
  const speciesOptions = useMemo(() => {
    ...
    return list.map((sp: Species) => ({ value: sp.id.toString(), label: sp.name }));
  }, [rawSpecies]);
...
const handleSubmit = async (
  ...
  try {
    ...
    toast.error(error?.meta?.message || error?.message || "Erreur lors de l'opération");
  } finally {
    setSubmitting(false);
  }
};
    ...
    <div className="max-w-full">
      ...
      <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
        ...
        <p className="text-sm text-green-800 font-medium">
          Ferme : <span className="font-bold">{farms.find((f) => f.id === finalFarmId)?.name || "Chargement..."}</span>
        </p>
      </div>
        ...
        {({ values, setFieldValue, errors, touched, isSubmitting }) => (
          <Form className="space-y-4">
            ...
            <div className="mb-6 flex flex-col items-center">
              <label className="text-sm font-bold text-gray-700 mb-2 w-full">Photo de l'animal</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                />
                {touched.speciesId && errors.speciesId && <p className="text-xs text-red-500">{errors.speciesId}</p>}
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium">Race (optionnelle)</label>
                <SelectInput
                  ...
                  placeholder={values.speciesId ? "Rechercher une race" : "Sélectionnez l'espèce"}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label="Date de naissance" name="birthDate" type="date" required />
              <Input label={initialData ? "Poids actuel (kg)" : "Poids initial (kg)"} name="weight" type="number" placeholder="0.00" required />
            </div>
